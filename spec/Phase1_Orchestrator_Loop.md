...
| **1.6b** | **Rollen-Scoping** (least-privilege Ausführung): Job-Service läuft unter `ORCH_PROJ_<ID>` — `ORCH_RUNNER` an User, PROJ-Rolle: USAGE on Pool, READ Image-Repo, CREATE SERVICE, Stage-Mount-Zugriff; kein ACCOUNTADMIN mehr. | Loop läuft als `ORCH_PROJ_<ID>` (verifiziert über CURRENT_ROLE im Log) |
